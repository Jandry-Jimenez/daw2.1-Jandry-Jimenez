<?php
	...
	$edad = $_REQUEST["edad"];
    $cancionId = (int)$_REQUEST["cancionId"];
	...
	if ($nuevaEntrada) {
		// Quieren CREAR una nueva entrada, así que es un INSERT.
 		$sql = "INSERT INTO artista (nombre, apellidos, edad, estrella, cancionId) VALUES (?, ?, ?, ?, ?)";
        $parametros = [$nombre, $apellidos, $edad, $estrella?1:0, $cancionId];
	} else {
		// Quieren MODIFICAR una persona existente y es un UPDATE.
 		$sql = "UPDATE artista SET nombre=?, apellidos=?, edad=?, estrella=?, cancionId=? WHERE id=?";
        $parametros = [$nombre, $apellidos, $edad, $estrella?1:0, $cancionId, $id];
 	}
...
<a href='artistaListado.php'>Volver al listado artistas.</a>
