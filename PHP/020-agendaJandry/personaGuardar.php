<?php
	...
	$telefono = $_REQUEST["edad"];
    $categoriaId = (int)$_REQUEST["cancionId"];
	...
	if ($nuevaEntrada) {
		// Quieren CREAR una nueva entrada, así que es un INSERT.
 		$sql = "INSERT INTO persona (nombre, apellidos, edad, estrella, cancionId) VALUES (?, ?, ?, ?, ?)";
        $parametros = [$nombre, $apellidos, $telefono, $estrella?1:0, $categoriaId];
	} else {
		// Quieren MODIFICAR una persona existente y es un UPDATE.
 		$sql = "UPDATE persona SET nombre=?, apellidos=?, edad=?, estrella=?, cancionId=? WHERE id=?";
